...
export default function StoryboardPage() {
  ...
  function pickMulti(id: string) { setSele
cd ~/Downloads/soon-storyboard
cat > src/app/storyboard/page.tsx << 'ENDOFFILE'
'use client'
import { useState } from 'react'
import { STEPS, Step, ShotOption } from './data/shots'
import Image from 'next/image'
...
export default function StoryboardPage() {
  ...
  function doneCount() { return STEPS.filter(isDone).length }
  function pickSingle(id: string) { setSelections(p => ({ ...p, [step.id]: id })) }
  function pickMulti(id: string) { setSelections(p => { const c = (p[step.id] as string[]) || []; const e = c.includes(id); return { ...p, [step.id]: e ? c.filter(x => x !== id) : [...c, id] } }) }
  function getLabels(s: Step) { const sel = selections[s.id]; if (s.type === 'single') { const o = s.options.find(o => o.id === sel); return o ? [o.name] : ['（未選）'] } if (Array.isArray(sel) && sel.length > 0) return sel.map(id => s.options.find(o => o.id === id)?.name ?? id); return ['（未選）'] }
  ...
  async function generateDocx() {
    ...
    try {
      ...
      const td = new Date(), ds = `${td.getFullYear()}.${String(td.getMonth()+1).padStart(2,'0')}.${String(td.getDate()).padStart(2,'0')}`
      ...
      STEPS.forEach(s => {
        ...
        getLabels(s).forEach(l => ch.push(new Paragraph({ children: [new TextRun({ text: `· ${l}`, font: 'Arial', size: 22 })], spacing: { after: 50 }, indent: { left: 240 } })))
    ...
    } catch(e) { console.error(e); setGenStatus('生成失敗，請重試') }
    ...
    <div style={{minHeight:'100vh',background:cr,color:ink,fontFamily:'Georgia,serif'}}>
      ...
      {showSummary ? (
        <div style={{padding:24}}>
          ...
          {STEPS.map((s,i) => <div key={s.id} style={{marginBottom:24,paddingBottom:24,borderBottom:i<STEPS.length-1?`0.5px solid ${br}`:'none'}}>
            ...
            <div style={{display:'flex',flexWrap:'wrap',gap:6}}>{getLabels(s).map(l => <span key={l} style={{fontFamily:'system-ui,sans-serif',fontSize:11,padding:'3px 10px',border:`0.5px solid ${br}`,borderRadius:999,background:hv,color:ink}}>{l}</span>)}</div>
          </div>)}
          ...
          {step.type==='single' ? (
            <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:8,marginBottom:24}}>
              {step.options.map((o,j) => {
                ...
                return <button key={o.id} onClick={()=>pickSingle(o.id)} style={{textAlign:'left',border:`0.5px solid ${sel?ink:br}`,borderRadius:5,padding:'11px 10px',background:sel?hv:'transparent',cursor:'pointer'}}>
                  ...
                  <p style={{fontSize:12,fontWeight:500,color:ink,marginBottom:2,margin:'0 0 2px'}}>{o.name}</p>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:6,marginBottom:24}}>
              {step.options.map(o => {
                ...
                return <button key={o.id} onClick={()=>pickMulti(o.id)} style={{display:'flex',alignItems:'flex-start',gap:10,padding:'10px 12px',border:`0.5px solid ${on?ink:br}`,borderRadius:5,background:on?hv:'transparent',cursor:'pointer',textAlign:'left'}}>
                  ...
                  <span><p style={{fontSize:14,color:ink,margin:'0 0 2px'}}>{o.name}</p><p style={{fontFamily:'system-ui,sans-serif',fontSize:10,color:mu,margin:0}}>{o.description}</p></span>
                </button>
              })}
            </div>
          )}
