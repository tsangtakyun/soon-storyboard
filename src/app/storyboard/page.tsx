...
export default function StoryboardPage() {
  ...
  function isDone(s: Step) { const sel = selections[s.id]; if (s.type === 'single') return typeof sel === 'string' && sel !== ''; return Array.isArray(sel) && sel.length > 0 }
  function doneCount() { return STEPS.filter(isDone).length }
  function pickSingle(id: string) { setSelections(p => ({ ...p, [step.id]: id })) }
  function pickMulti(id: string) { setSelections(p => { const c = (p[step.id] as string[]) || []; const e = c.includes(id); return { ...p, [step.id]: e ? c.filter((x: string) => x !== id) : [...c, id] } }) }
  function getLabels(s: Step) { const sel = selections[s.id]; if (s.type === 'single') { const o = s.options.find(o => o.id === sel); return o ? [o.name] : ['（未選）'] } if (Array.isArray(sel) && sel.length > 0) return sel.map((id: string) => s.options.find(o => o.id === id)?.name ?? id); return ['（未選）'] }
  ...
  return (
    <div style={{minHeight:'100vh',background:cr,color:ink,fontFamily:'Georgia,serif'}}>
      ...
      <div style={{padding:24}}>
        ...
        {step.type==='single'?(<div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:8,marginBottom:24}}>{step.options.map(o=>{const sel=selections[step.id]===o.id;return(<button key={o.id} onClick={()=>pickSingle(o.id)} style={{textAlign:'left',border:`0.5px solid ${sel?ink:br}`,borderRadius:5,padding:'11px 10px',background:sel?hv:'transparent',cursor:'pointer'}}>{o.img?<div style={{position:'relative',width:'100%',aspectRatio:'9/16',marginBottom:8,borderRadius:3,overflow:'hidden'}}><Image src={o.img} alt={o.name} fill style={{objectFit:'cover'}} sizes="200px"/></div>:<div style={{width:'100%',aspectRatio:'9/16',marginBottom:8,borderRadius:3,background:hv,display:'flex',alignItems:'center',justifyContent:'center'}}><span style={{fontFamily:'system-ui,sans-serif',fontSize:10,color:mu}}>待補充</span></div>}<p style={{fontSize:12,fontWeight:500,color:ink,margin:'0 0 2px'}}>{o.name}</p><p style={{fontFamily:'system-ui,sans-serif',fontSize:10,color:mu,lineHeight:1.4,margin:0}}>{o.description}</p></button>)})}</div>):(<div style={{display:'flex',flexDirection:'column',gap:6,marginBottom:24}}>{step.options.map(o=>{const on=Array.isArray(selections[step.id])&&(selections[step.id] as string[]).includes(o.id);return(<button key={o.id} onClick={()=>pickMulti(o.id)} style={{display:'flex',alignItems:'flex-start',gap:10,padding:'10px 12px',border:`0.5px solid ${on?ink:br}`,borderRadius:5,background:on?hv:'transparent',cursor:'pointer',textAlign:'left'}}><span style={{width:14,height:14,borderRadius:3,border:`0.5px solid ${on?ink:br}`,background:on?ink:'transparent',flexShrink:0,marginTop:3,display:'flex',alignItems:'center',justifyContent:'center'}}>{on&&<svg width="8" height="6" viewBox="0 0 8 6" fill="none"><path d="M1 3L3 5L7 1" stroke="#F5F2EC" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>}</span><span><p style={{fontSize:14,color:ink,margin:'0 0 2px'}}>{o.name}</p><p style={{fontFamily:'system-ui,sans-serif',fontSize:10,color:mu,margin:0}}>{o.description}</p></span></button>)})}</div>)}
...
ENDOFFILE\
tee ~/Downloads/soon-storyboard/src/app/storyboard/page.tsx > /dev/null << 'ENDOFFILE'
'use client'
import { useState } from 'react'
import { STEPS, Step } from './data/shots'
import Image from 'next/image'
